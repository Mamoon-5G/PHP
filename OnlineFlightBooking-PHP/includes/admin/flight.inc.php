    ...
    if($flag == false) {
      ...
      if($row = mysqli_fetch_assoc($result)) {
        ...
        $sql = "INSERT INTO Flight(admin_id,arrivale,departure,Destination,source,
          airline,pilot,Seats,duration,Price,status,issue) VALUES (?,?,?,
          ?,?,?,?,?,?,?,'','')";
        ...
        if(!mysqli_stmt_prepare($stmt,$sql)) {
          ...
          mysqli_stmt_bind_param($stmt,'issssssisi',$admin_id,$arrival,$departure,$arr_city
            ,$dep_city,$airline_name,$pilot,$seats,$dura,$price);            
          mysqli_stmt_execute($stmt); 
        }
        mysqli_stmt_close($stmt);
        mysqli_close($conn);
